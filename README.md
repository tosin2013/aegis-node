...
**Status:** Phase 1c complete — [**v0.9.0** *Tooling and Replay*](https://github.com/tosin2013/aegis-node/releases/tag/v0.9.0) (pre-release, 2026-05-05). Builds on [v0.8.0](https://github.com/tosin2013/aegis-node/releases/tag/v0.8.0) with F8 [trajectory replay viewer](https://github.com/tosin2013/aegis-node/issues/61) (deterministic offline single-file HTML), F10 [`aegis validate`](https://github.com/tosin2013/aegis-node/issues/58) (schema validator + linter, ADR-012), [OCI model pull + cosign verify](https://github.com/tosin2013/aegis-node/issues/65) via `aegis pull` (ADR-013), [llama.cpp Rust FFI](https://github.com/tosin2013/aegis-node/issues/69) under the Backend trait (ADR-014), [LiteRT-LM second backend](docs/adrs/023-litertlm-second-inference-backend.md) for Gemma 4 (ADR-023), [MCP client adoption](docs/adrs/018-adopt-mcp-protocol-for-agent-tool-boundary.md) with [ADR-024 `pre_validate`](docs/adrs/024-mediator-pre-validation-for-mcp-tool-args.md) for per-tool arg validation, and six [fork-friendly examples](examples/) mapped to OWASP Top 10 for Agentic Applications 2026. Phase 1 GA (v1.0.0) in progress — multi-turn agent loop with per-turn enforcement (ADRs [025](docs/adrs/025-multi-turn-agent-loop-with-triple-bound-circuit-breaker.md)–[030](docs/adrs/030-per-turn-spiffe-mtls-attestation.md), informed by the [security research brief](docs/research/multi-turn-agent-loop.md)), web UI, conformance suite, `aegis evidence cmmc` evidence-pack generator (per [docs/COMPLIANCE_MATRIX.md](docs/COMPLIANCE_MATRIX.md)), MITRE ATLAS red-team validation, and design-partner security review. v1.0.0 targets the [CMMC 2.0 deadline 2026-11-02](RELEASE_PLAN.md).
