#!/bin/sh
# Demo 05 setup — stage everything `demo.tape` expects into
# /tmp/aegis-demo-05/. Idempotent; safe to re-run. Invoked
# automatically by `make 05-tampered-model-halts` (per
# demos/Makefile's `%/demo.gif` rule).
#
# Per ADR-020 + ADR-003. Uses the llama backend + Qwen2.5-1.5B-
# Instruct Q4_K_M; the model file gets *copied* (not symlinked) so
# the demo's atomic-rename tamper (`mv /tmp/t.bin model.gguf`)
# doesn't touch the cache. The committed cache stays intact for the
# next render.

set -eu

WORKDIR=/tmp/aegis-demo-05
...
mkdir -p "$WORKDIR"
# Copy (not symlink) — the .tape's `mv /tmp/t.bin model.gguf` would
# replace the symlink target, corrupting the cache. A copy isolates
# the tamper to the workdir.
cp -f "$CACHED/blob.bin" "$WORKDIR/model.gguf"
...
# Pre-clean ledger + run.log from any prior render so the .tape's
# `rm -f ledger-*.jsonl` is a no-op for the demo viewer.
rm -f "$WORKDIR"/ledger-*.jsonl "$WORKDIR/run.log" /tmp/t.bin

echo '[05] ready: cd '"$WORKDIR"' && vhs '"$(dirname "$0")"'/demo.tape'
