...
- [x] **P0** [ADR-018, F2/F5] Adopt MCP as the agent-to-tool protocol per ADR-018. Manifest gains optional `tools.mcp[]` (closed-by-default allowlist of `{server_name, server_uri, allowed_tools}`); mediator gains `mediate_mcp_tool_call` that routes side-effects through existing `mediate_*` methods; cross-language conformance battery extended; example manifest under `schemas/manifest/v1/examples/agent-with-mcp.manifest.yaml` uses Anthropic's reference filesystem MCP server. — PR #54 (#43 schema), PR #55 (#44 mediator + new `crates/mcp-client`), PR #56 (#45 conformance), PR #57 (#46 example). Umbrella #42 closed. **Out of scope for v0.9.0:** F5 wiring of MCP tool names into `toolsConsidered`/`toolSelected`, HTTP/SSE transport, approval-gate routing for MCP calls.
